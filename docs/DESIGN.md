...
Rustの標準的な慣習に従い、各ソースファイル内に `#[cfg(test)]` 属性を持つテストモジュール `tests` を配置する。
...
- **境界値**: 入力文字数0、経過時間0、ミス数0など。
  - *注意*: 経過時間0の場合のゼロ除算挙動を確認する（Rustでは `f64` のゼロ除算は `inf` になる）。
...
- 現状、乱数生成器が注入できない設計になっているため、厳密なロジックテスト（特定のシードでの出力固定など）は行わない。将来的にRNGをDIするリファクタリングを行う際にテストを強化する。
